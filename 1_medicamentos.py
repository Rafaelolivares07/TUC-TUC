...
def setup_festivos_table():
    """Crea la tabla festivos e inserta festivos de Colombia 2026"""
    try:
        conn = get_db_connection()
        ...
        count_antes = conn.execute("SELECT COUNT(*) as cnt FROM festivos").fetchone()['cnt']

        # Insertar festivos de Colombia 2026 (si no existen)
        festivos_2026 = [
            ('2026-01-01', 'Año Nuevo'),
            ('2026-01-12', 'Día de los Reyes Magos'),
            ('2026-03-23', 'Día de San José'),
        ...
        # Contar después
        count_despues = conn.execute("SELECT COUNT(*) as cnt FROM festivos").fetchone()['cnt']
        insertados = count_despues - count_antes

        # Mostrar festivos actuales
        festivos = conn.execute("SELECT fecha, nombre FROM festivos WHERE activo = TRUE ORDER BY fecha").fetchall()
        festivos_html = "".join([f"<li>{f['fecha']} - {f['nombre']}</li>" for f in festivos])

        return f"""
        <h1>✅ Tabla festivos configurada</h1>
        <p>Tabla creada correctamente</p>
        <p>Índices creados</p>
        <p>Festivos existentes antes: {count_antes}</p>
        <p>Festivos insertados: {insertados}</p>
        <p>Total festivos activos: {count_despues}</p>
        <h3>Festivos 2026:</h3>
        <ul>{festivos_html}</ul>
        <br>
        <a href="/admin/parametros">Volver a Parámetros</a>
        """
    except Exception as e:
        import traceback
        traceback.print_exc()
        ...
        if is_logged_in:
            ...
            items_carrito = conn.execute("""
                SELECT
                    ...
                    e.medicamento_id as id_medicamento,
                    e.cantidad,
                    e.precio_unitario,
                    e.precio_total,
                    m.nombre as nombre_comercial,
                    m.presentacion as nombre_generico,
                    m.imagen
                FROM existencias e
                JOIN medicamentos m ON e.medicamento_id = m.id
                WHERE e.estado = 'carrito_temporal'
...
@app.route('/api/diagnosticar-diagnosticos-db', methods=['GET'])
def diagnosticar_diagnosticos_db():
    """ENDPOINT TEMPORAL: Diagnostica y corrige estructura de tabla diagnosticos"""
    try:
        import psycopg2
        database_url = os.getenv('DATABASE_URL')
        if database_url and database_url.startswith('postgres://'):
        ...
        # 1. Ver estructura actual de DIAGNOSTICOS
        cursor.execute("""
            SELECT column_name, data_type, column_default, is_nullable
            FROM information_schema.columns
        ...
        cursor.execute("""
            SELECT c.column_default
            FROM information_schema.columns c
            WHERE c.table_name = 'DIAGNOSTICOS' AND c.column_name = 'id'
        """)
        result = cursor.fetchone()
        column_default = result[0] if result else None
        ...
        if not secuencia:
            # Crear secuencia
            cursor.execute("CREATE SEQUENCE IF NOT EXISTS diagnosticos_id_seq")

            # Obtener el máximo id actual
            cursor.execute('SELECT COALESCE(MAX(id), 0) + 1 FROM "DIAGNOSTICOS"')
            next_val = cursor.fetchone()[0]

            # Establecer el valor inicial de la secuencia
            cursor.execute(f"ALTER SEQUENCE diagnosticos_id_seq RESTART WITH {next_val}")

            # Asociar la secuencia a la columna id
            cursor.execute('ALTER TABLE "DIAGNOSTICOS" ALTER COLUMN id SET DEFAULT nextval(\'diagnosticos_id_seq\')')

            # Hacer que la secuencia sea propiedad de la columna
            cursor.execute('ALTER SEQUENCE diagnosticos_id_seq OWNED BY "DIAGNOSTICOS".id')
        ...
        cursor.execute("""
            SELECT column_name, data_type, column_default, is_nullable
            FROM information_schema.columns
            WHERE table_name = 'DIAGNOSTICOS' AND column_name = 'id'
        """)
        columna_id = cursor.fetchone()
        resultados.append({
            'paso': 'Verificación final',
    ...
    try:
        ...
        if busqueda_sintomas:
            ...
            normalizaciones_sintomas = {
                ...
                'pica': 'picazon', 'picando': 'picazon', 'picar': 'picazon',
                'vomito': 'vomito', 'vomitando': 'vomito', 'vomitar': 'vomito',
...
def obtener_admins():
    ...
    try:
        ...
        # Buscar terceros relacionados con usuarios que tienen rol Administrador
        admins = conn.execute("""
...
@app.route('/api/restaurante/<slug>/pedidos')
def api_restaurante_pedidos(slug):
    """Listar pedidos (filtrable por estado)"""
...
@app.route('/api/restaurante/<slug>/pedido/<int:pedido_id>/listo', methods=['POST'])
def api_restaurante_pedido_listo(slug, pedido_id):
    ...
    try:
        ...
        conn.execute(
            "UPDATE pedidos_restaurante SET estado = 'listo' WHERE id = %s AND restaurante_id = %s AND estado = 'pendiente'",
            (pedido_id, rest['id'])
        )
        conn.commit()
        conn.close()
...
@app.route('/api/restaurante/<slug>/pedido/<int:pedido_id>/entregado', methods=['POST'])
def api_restaurante_pedido_entregado(slug, pedido_id):
    ...
    try:
        ...
        conn.execute(
            "UPDATE pedidos_restaurante SET estado = 'entregado' WHERE id = %s AND restaurante_id = %s AND estado = 'listo'",
            (pedido_id, rest['id'])
        )
        conn.commit()
        conn.close()
...
@app.route('/api/restaurante/<slug>/agotar/<int:opcion_id>', methods=['POST'])
def api_restaurante_agotar(slug, opcion_id):
    """Marcar opción como agotada en el menú de hoy (cocina)"""
    try:
        conn = get_db_connection()
        crear_tablas_restaurante(conn)
        rest = conn.execute("SELECT id FROM restaurantes WHERE slug = %s", (slug,)).fetchone()
        ...
        if mdo:
            conn.execute(
                "UPDATE menu_dia_opciones SET agotado = NOT agotado WHERE id = %s", (mdo['id'],)
            )
        conn.commit()
