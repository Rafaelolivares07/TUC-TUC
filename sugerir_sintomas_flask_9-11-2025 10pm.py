...
<head>
  ...
  <script>
    ...
    function irAMedicamento() {
      ...
      // Marcar que el cambio viene desde la app (para NO auto-pegar)
      localStorage.setItem('cambioMedicamentoInterno', '1');

      // Limpiar textarea visualmente antes de recargar
      const textarea = document.getElementById('textoExtraido');
      if (textarea) {
        textarea.value = '';
      }
      ...
      if (diagnosticosContainer) {
        diagnosticosContainer.innerHTML = '<p style="color: #999;">Cargando diagnósticos...</p>';
      }
      
      const sintomasContainer = document.getElementById('sintomas-container');
      if (sintomasContainer) {
        sintomasContainer.innerHTML = '<p style="color: #999;">Cargando síntomas...</p>';
      }
      
      localStorage.setItem('cambioMedicamentoInterno', '1'); // ✅ MARCA

      const skipScraping = document.getElementById('skipScraping').checked ? '1' : '0';
      window.location = '/sugerir-sintomas/ver/' + medId + '?skip_scraping=' + skipScraping;
    }
  </script>
</head>
<body>
  <div class="container">
    ...
    <div class="seccion">
      ...
      <textarea id="textoExtraido" placeholder="Pega aquí el contenido que copiaste..." style="height: 200px;">{{ texto }}</textarea>
    ...
    <form method="post" action="/sugerir-sintomas/guardar/{{ med.id }}">
      ...
      <!-- BOTÓN GUARDAR -->
      <button type="submit" style="width: 100%; padding: 12px; margin-top: 20px; font-size: 16px;">
...
def ver_sugerir_med(med_id):
    ...
    todos_meds = db.execute('''
        ...
        AND id NOT IN (SELECT medicamento_id FROM medicamento_sintoma)
        ORDER BY nombre
    ''').fetchall()
    
    db.close()
    return render_template('poblacion_medicamentos.html', 
                          meds=todos_meds,
                          med=med, 
                          termino=termino_limpio, 
                          texto=texto, 
                          fuente=fuente, 
                          sugestiones=sugestiones_db,
