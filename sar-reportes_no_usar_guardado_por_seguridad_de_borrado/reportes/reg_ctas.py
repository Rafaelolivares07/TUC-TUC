...
def tablas_requeridas(filtros):
    ...
    def a_lapso_desde(fecha_iso):
        return fecha_iso.replace('-', '') if fecha_iso else ''

    def a_lapso_hasta(fecha_iso):
        return fecha_iso.replace('-', '') if fecha_iso else ''
    ...
    if desde or hasta:
        filtros_rc['LAPSO'] = {}
        if desde: filtros_rc['LAPSO']['desde'] = a_lapso_desde(desde)
        if hasta: filtros_rc['LAPSO']['hasta'] = a_lapso_hasta(hasta)

    return [
        {
            'tabla':  'REG_CTAS',
            'campos': ['CONSECUTIV', 'FECHAHORA', 'LAPSO', 'TIPO', 'DOCUMENTO',
                       'CUENTA', 'TERCERO', 'DEBITO', 'CREDITO', 'VALOR',
                       'DETALLE', 'ANULADO', 'EMPRESA'],
            'filtros': filtros_rc,
        },
        {
            'tabla':   'TERCEROS',
...
def calcular(datos, filtros):
    ...
    for r in datos.get('REG_CTAS', []):
        ...
        rows.append({
            ...
            'debito':      round(float(r.get('DEBITO',  0) or 0) or float(r.get('VALOR', 0) or 0), 2),
