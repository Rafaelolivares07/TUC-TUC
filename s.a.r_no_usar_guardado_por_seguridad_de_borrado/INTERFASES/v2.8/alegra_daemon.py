"""
alegra_daemon.py  v2.2
Servicio en segundo plano — corre allegra_sync.py + interfaz_allegra.py cada N minutos.
N se lee de allegra_config.dbf en cada ciclo (cambios en el form se aplican sin reiniciar).
Si intervalo=0, espera 5 minutos y vuelve a revisar.

Uso:  python C:\\S.A.R\\alegra_daemon.py
Build: pyinstaller --onefile --noconsole --name AlegraDaemon alegra_daemon.py
Startup: acceso directo de AlegraDaemon.exe en shell:startup
"""
...
# Cuando está compilado como .exe, sys.executable apunta al propio exe.
# Buscamos el intérprete Python real en el PATH.
def _python_exe():
...
ESPERA_MIN    = 5   # minutos de espera cuando intervalo=0

SAR_DIR = r"C:\S.A.R"
if SAR_DIR not in sys.path:
...
def _actualizar_estado_json():
    ...
    try:
        ...
        # Fases activas — leer desde interfaz_allegra.py (fuente de verdad)
        try:
...
def main():
    ...
    kernel32.CreateMutexW(None, True, "Global\\AlegraDaemonMutex22")
