from flask import Flask, request, redirect, url_for,send_from_directory, jsonify, render_template, session, flash
import sqlalchemy
import pandas  
import sqlite3
import uuid
...
app.config['SECRET_KEY'] = 'mi_clave_secreta_para_probar_el_acceso_temporal'
...
def admin_required(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        print(f"🔐 DECORADOR EJECUTADO para función: {f.__name__}")
        print(f"   📋 Sesión actual: {dict(session)}")
        print(f"   🎭 Rol en sesión: {session.get('rol', 'NO TIENE ROL')}")
        ...
        print(f"   ✅ ACCESO PERMITIDO - Ejecutando función {f.__name__}")
...
def verificar_match_sintoma(nombre_sintoma, palabras_usuario, umbral=0.8):
    ...
    """
    palabras_sintoma = nombre_sintoma.lower().split()
            ...
            try:
                ...
                for variante in variantes:
                    query = """
                        ...
                        LIMIT 10
                    """
                    resultados = conn.execute(query, [f'%{variante_normalizada}%', f'%{variante_normalizada}%']).fetchall()
...
def fusionar_sintomas():
    """Fusiona varios síntomas en uno solo, migrando todos los medicamentos"""
    conn = None
    try:
        data = request.get_json()
        ...
        # Eliminar asociaciones de los síntomas secundarios
        for sintoma_id in sintomas_secundarios_ids:
            cursor.execute("""
        ...
        return jsonify({
            'ok': True,
            'mensaje': f'✅ Síntomas fusionados exitosamente. Se agregaron {len(medicamentos_nuevos)} medicamentos nuevos.'
        })
...
def buscar_o_crear_tercero():
    ...
    if tercero:
        # Actualizar fecha_actualizacion al seleccionar tercero existente
        db.execute(
...
def guardar_configuracion_precios():
    ...
    campos_validos = ['usar_precio', 'recargo_escaso', 'redondeo_superior', 
                      'ganancia_min_escaso', 'ganancia_max_escaso']
...
def api_pastillero():
    ...
    try:
        ...
        if orden == 'alfabetico':
            order_by = 'ORDER BY p.nombre ASC'
...
def api_pastillero_tomar(medicamento_id):
    ...
    try:
        ...
        medicamento = conn.execute('''
            SELECT cantidad FROM pastillero_usuarios
        ...
        conn.commit()
        conn.close()
        return jsonify({'ok': True, 'nueva_cantidad': max(0, nueva_cantidad)})
    except Exception as e:
        conn.rollback()
        conn.close()
